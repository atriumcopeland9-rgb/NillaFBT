...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var overlay: SkeletonOverlayView
    ...
    private var lastFrameHadPose = false

    private val landmarkSmoother = LandmarkSmoother()
    private lateinit var imuGate: ImuMotionGate
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        previewView.scaleType = PreviewView.ScaleType.FIT_CENTER // must match SkeletonOverlayView's scaling assumption
        overlay = findViewById(R.id.skeletonOverlay)
        overlay.mirror = true // front camera - flip to match the mirrored preview
        ...
        findViewById<Button>(R.id.btnToggle).setOnClickListener { btn ->
            ...
            if (!tracking) {
                ...
                landmarkSmoother.reset() // fresh start each session, avoid stale filter state
                statusText.text = "Tracking... (no pose detected yet)"
            }
        }
        ...
        imuGate = ImuMotionGate(this)
        imuGate.start()

        poseProcessor = PoseProcessor(
            context = this,
            onResult = { worldLandmarks, visibilities -> onPose(worldLandmarks, visibilities) },
            onImagePose = { imagePoints, w, h -> onImagePose(imagePoints, w, h) },
            onError = { msg -> runOnUiThread { statusText.text = "PoseLandmarker error: $msg" } },
            onNoPose = { runOnUiThread {
                if (lastFrameHadPose) overlay.clear()
                lastFrameHadPose = false
                if (tracking) statusText.text = "Tracking... (no pose detected)"
            } }
        )
    ...
    private fun onImagePose(imagePoints: List<ImagePoint>, w: Int, h: Int) {
        runOnUiThread {
            lastFrameHadPose = true
            if (tracking) statusText.text = "Tracking... pose detected"
            overlay.update(imagePoints, w, h)
        }
    }

    private fun onPose(worldLandmarks: List<Vec3>, visibilities: List<Float>) {
        val smoothed = landmarkSmoother.smooth(
            worldLandmarks,
            visibilities,
            System.currentTimeMillis(),
            imuGate.isCameraMoving
        )
        val points = BodyMapper.map(smoothed)
